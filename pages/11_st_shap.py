# in house shap library?

# this code is sketchy, it'll need to be fixed.
...
@st.cache_data
def load_model(X, y):
    # just splitting data into test/train
...
with st.expander("Force plot"):
    ...
    st_shap(shap.plots.force(explainer.expected_value, shap_values[0,:], X_display.iloc[0,:]), height=200, width=1000)
    st.subheader("First thousand data instance")
    st_shap(shap.plots.force(explainer.expected_value, shap_values[:1000,:], X_display.iloc[:1000,:]), height=400, width=1000)